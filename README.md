...
| 영역 | 선택 | 이유 |
|---|---|---|
| Framework | LangChain(LCEL) → **LangGraph** | 조건부 분기까지는 LCEL로도 되지만, 되돌아오는 사이클과 실행 중단·재개(`interrupt`)는 표현할 수 없었다 |
| LLM | Gemini 3.5 Flash | Structured Output으로 응답 스키마를 강제해, 자유 텍스트 파싱 없이 API 계약을 지킨다 |
| Vector DB | Chroma (`hnsw:space=cosine`) | 텍스트 임베딩은 크기가 아니라 방향이 의미를 담으므로 기본값 L2 대신 코사인 거리를 지정 |
| Embedding (KB) | **Gemini Embedding** | KB를 29개로 늘리자 `ko-sroberta`가 "멱등성", "N+1" 같은 저빈도 용어를 구분하지 못했다. 30문항 기준 86.7% → 100% |
| Embedding (업로드 문서) | ko-sroberta (로컬) | 이력서는 사용자가 올린 개인정보라 외부 API로 보내지 않는다 |
| Backend | FastAPI | ASGI 기반이라 LLM 호출처럼 대기가 긴 I/O에서 처리량이 나온다 |
| Frontend | 순수 HTML/CSS/JS | 단일 페이지 데모에 SPA 프레임워크는 과하다고 보고 배제. 빌드 단계·CORS 설정이 없다 |
| 패키지 관리 | uv | `uv.lock`으로 버전을 고정해 로컬과 컨테이너의 의존성을 일치시킨다 |
| 배포 | Docker → Docker Hub → EC2 | GitHub Actions에서 빌드해 이미지를 올리고, EC2는 받아서 띄우기만 한다 |

**평가 도구**는 목적이 서로 다른 세 가지를 나눠 씁니다.

| 도구 | 무엇을 재는가 | 현재 값 |
|---|---|---|
| Retrieval 평가셋 (65문항 5유형) | 검색이 올바른 근거를 찾는가 | in-scope 38문항 Top-1 100%, Faithfulness 0.9702 |
| Judge Calibration Set (44개) | 채점이 올바른 코칭 경로로 가는가 | 경로 정확도 90.9% (변동 폭 약 5%p) |
| RAGAS Faithfulness | 생성한 내용이 근거에 충실한가 | Calibration Set 기준 평균 0.4412 |

## Project Structure

```
├── app/main.py              FastAPI 진입점. 라우터 등록과 정적 파일 서빙
├── api/                     엔드포인트. 라우팅과 응답 조립만 담당하고 로직은 rag/에 둔다
│   ├── documents.py           POST /documents          문서 업로드·인덱싱
│   ├── questions.py           POST /generate-question  Chain A
│   ├── evaluations.py         POST /evaluate-answer    Chain B (단발성)
│   └── interviews.py          POST /interview/*        멀티턴 세션
│
├── rag/                     RAG 파이프라인의 실제 로직
│   ├── graph.py               그래프 배선. 노드는 공유하고 배선만 다른 5종
│   ├── graph_nodes.py         노드 구현과 분기 함수 (decide_scope, decide_next_step)
│   ├── graph_state.py         노드 사이로 오가는 State 정의
│   ├── schemas.py             Structured Output 스키마. LLM 응답 형식을 강제한다
│   ├── prompts.py             프롬프트. 채점 루브릭이 여기 있다
│   ├── vectorstore.py         Chroma 연결. 앱이 쓸 검색기를 여기 한 곳에서 정한다
│   ├── embeddings.py          임베딩 모델 (Gemini / ko-sroberta)
│   ├── loader.py              문서 로딩과 chunking
│   ├── llm_utils.py           LLM 응답 처리 유틸
│   ├── config.py              환경변수와 임계값
│   └── chains.py              마이그레이션 전 LCEL 구현. 과정을 남기려고 보존
│
├── kb/                      Interview KB. 운영자가 직접 작성한 고정 지식 29개 문서
├── static/                  프론트엔드 (순수 HTML/CSS/JS, 빌드 단계 없음)
│
├── tests/                   API 키 없이 도는 것만 둔다. CI에서 매번 실행
│   ├── test_agent_routing.py    분기 로직 전수 검증 + 코칭 누수 회귀
│   ├── test_multiturn_loop.py   노드를 가짜로 갈아끼워 사이클 증명
│   ├── test_eval_set.py         평가셋 정답 키가 KB와 일치하는지
│   ├── test_kb_indexing.py      인덱싱 대상이 앱의 검색 대상과 같은지
│   └── fixtures/                평가셋·캘리브레이션 세트·샘플 이력서
│
├── scripts/                 측정과 실험용. 앱 실행에는 쓰이지 않는다
│   ├── load_kb.py               KB 인덱싱 (배포 시 자동 실행)
│   ├── run_retrieval_eval.py    검색 품질 측정 (RAGAS)
│   ├── run_calibration_v2.py    채점 정확도 측정
│   ├── compare_embeddings_*.py  임베딩 비교
│   ├── calibrate_scope_threshold.py  범위 밖 임계값 산출 (group 5-fold)
│   ├── check_eval_set.py        평가셋 정답 키 검사 (API 불필요)
│   ├── eval_budget.py           호출량 확인과 --limit 옵션 공용 헬퍼
│   └── archive/                 더 쓰지 않지만 과정 기록으로 남긴 것
│
├── docs/
│   ├── experiment_log.md      시간순 실험 기록. 가설 → 방법 → 결과 → 결론
│   ├── project_spec_v1.md     설계 명세와 고도화 로드맵
│   └── reports/               유닉스 프로세스 분석, WireShark 캡처
│
├── Dockerfile               CPU 전용 torch로 이미지 축소
├── docker-entrypoint.sh     기동 시 KB 인덱싱 여부 판단
├── docker-compose*.yml      로컬 / EC2 배포용
└── pyproject.toml           의존성 (uv, uv.lock으로 버전 고정)
```

`chroma_db/`(벡터 인덱스)와 `data/uploads/`(업로드 파일)는 실행 중에 생기는 데이터라 저장소에 포함하지 않고, 배포 환경에서는 Docker volume으로 보존합니다.

**`api/`와 `rag/`를 나눈 이유**는 검증 지점을 분리하기 위해서입니다. `rag/`의 분기 함수는 State만 받는 순수 함수라 LLM 호출 없이 전수 검증이 되고, `api/`는 응답 조립만 하므로 스키마 변경의 영향 범위가 좁습니다.

**`scripts/`는 앱이 아니라 실험 도구입니다.** 이 프로젝트에서 결론이 세 번 뒤집혔는데, 매번 같은 스크립트를 다시 돌려 확인할 수 있었던 것이 재현성의 근거였습니다. 다만 대부분 Gemini API를 쓰므로 `eval_budget.py`가 예상 호출량을 먼저 알려주고 `--limit`으로 줄여 돌릴 수 있게 했습니다.
...
- 근거가 없는 질문은 채점을 보류하는 `out_of_scope` 분기 추가. 임계값은 65문항 평가셋에서 group 5-fold 교차 검증으로 결정
...
*"좋아졌다"는 말을 언제 믿을 수 있는가. 이 프로젝트에서 결론이 세 번 뒤집혔고, 매번 원인은 측정 도구였습니다.*
...
범위 밖 문항을 19개 넣었더니 범위 안(0.166~0.271)과 범위 밖(0.324~0.412)이 **겹침 없이** 갈렸습니다. 좋은 결과로 보였지만, KB에서 뻔히 먼 주제만 골랐기 때문이었습니다. 어떤 임계값을 넣어도 100%가 나오면 임계값이 검증된 게 아닙니다. JPA 영속성 컨텍스트, SQL 인젝션처럼 **KB 가장자리에 걸치는 문항**을 넣고 나서야 겹침이 생겼고, 그제야 임계값 선택이 판단할 거리가 됐습니다.
...
*무엇을 고쳐야 검색이 좋아지는가. 문서를 고쳐서 되는 것과, 고쳐도 안 되는 것이 갈렸습니다.*
...
**근거가 없으면 채점하지 않는 것도 기능이다**

검색은 `k=3`이라 KB에 답이 없어도 항상 문서 3개를 돌려줍니다. `"좋아하는 음식이 무엇인가요?"`에도 `async_sync.md`가 나오고, 시스템은 그걸 근거로 점수를 매기고 있었습니다. 1순위 거리로 걸러내 **점수·레벨·학습 팁·꼬리질문을 아예 만들지 않고 보류**하도록 바꿨습니다. 틀린 근거로 매긴 점수를 사용자가 자기 실력으로 받아들이면 오히려 해롭기 때문입니다. 임계값 `0.311`은 65문항 평가셋에서 **group 5-fold 교차 검증**으로 정했습니다. 같은 주제를 표현만 바꾼 문항이 train/test에 걸치면 임계값이 실제보다 좋아 보여서, 주제 단위로 묶어 나눴습니다.

### 검증과 운영

*테스트가 통과해도 앱은 망가질 수 있습니다. 무엇을 자동화하고 무엇을 직접 봐야 하는가.*
...
**조용히 망가지는 고장은 자동 검사로 안 걸린다**

배포 하나에 버그 셋이 걸렸는데, 전부 **200으로 응답하고 테스트도 통과**했습니다. 인덱싱 대상이 앱의 검색 대상과 어긋나 EC2에서 모든 질문이 "근거 없음"으로 빠지던 건이 대표적입니다. 헬스체크도 멀쩡했습니다. 브라우저로 3턴을 직접 돌려보고, 배포 후 실제로 질문을 던져보고 나서야 드러났습니다. 지금은 셋 다 회귀 테스트로 묶어뒀습니다.
