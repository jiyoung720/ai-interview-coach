...
### 관찰 2-2. 17초의 네트워크 침묵: 지연의 원인 규명
이 캡처의 핵심은 패킷 5(0.016초)와 패킷 6(17.206초) 사이의 **약 17.19초 침묵**이다. 요청을 보낸 뒤 응답이 오기까지 네트워크에는 데이터 패킷이 거의 흐르지 않았다.

이는 "응답이 느린 이유가 무엇인가"에 대한 결정적 증거다. 만약 네트워크가 병목이었다면 이 17초 동안 패킷이 계속 오갔을 것이다. 조용하다는 것은 곧 **네트워크는 할 일이 없었다**는 뜻이며, 서버가 외부 LLM(Gemini) API의 응답을 기다리는 시간이었음을 보여준다. 앞선 과제 1(프로세스 상태 관찰)에서 이 구간 동안 서버 프로세스가 S(대기) 상태였던 것과 정확히 일치한다.

### 관찰 2-3. HTTP 평문 노출 (핵심 시연)
...
스마트폰이 세 번의 요청을 보냈고, 각각 **서로 다른 클라이언트 포트**로 관리됐다.

| 요청 | 엔드포인트 | Content-Type |
|---|---|---|
| POST /documents | 문서 업로드 | application/pdf |
| POST /generate-question | 질문 생성 | application/json |
| POST /evaluate-answer | 답변 평가 | application/json |

각 요청이 별도의 TCP 세션(포트 56252, 56256, 56258 등)으로 열리고 닫히는 것을 확인했다. 서버는 포트 번호 조합으로 각 연결을 구분해 관리한다.
...
### 향후 과제
- 서버에 HTTPS(TLS)를 적용한 뒤 같은 방식으로 캡처하면, 동일한 통신이 암호문으로만 보여 평문 HTTP와 대비된다. 이를 통해 HTTPS의 효과를 직접 확인할 수 있다.
