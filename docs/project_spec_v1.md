...
| KB가 기초~중간 난이도 (아키텍처 설계나 대규모 트래픽 주제 없음) | 실무 경험이 전제되지 않음 |
...
- **Phase 15 (KB 재설계)**: 난이도를 기초~중간으로 유지한다. 대규모 트래픽·분산 시스템 같은 주제는 넣지 않는다. 신입 면접에서 안 나오는 데다, 근거 없이 답하게 만들면 오히려 해롭다
...
- [x] **난이도는 기초~중간으로 유지** (Phase 13의 페르소나 결정). 대규모 트래픽·분산 시스템 등 신입 면접 범위 밖 주제는 넣지 않는다
