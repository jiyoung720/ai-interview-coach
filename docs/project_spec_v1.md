...
### Future Work / 선택
- [ ] **채점 루브릭 도입 (Phase 12 후보, 우선순위 높음)**
  - 배경: 현재 프롬프트는 `technical_score (0~10): 답변이 기술적으로 정확한가`가 전부라 **9점과 10점을 가르는 기준이 없다.** 그래서 같은 답변에 ±1점 변동이 생기고, Calibration 실패 케이스도 대부분 경계선 변동이었다. 사용자 입장에서도 "왜 이 점수인지" 납득하기 어렵다.
  - 내용: (1) 점수대별 판정 기준을 프롬프트에 명시, (2) 만점에서 깎아나가는 방식으로 **감점 사유와 감점 폭**을 함께 반환, (3) KB 근거 기반 **모범답안** 제공
  - 구현: `judge_node`가 이미 structured output을 쓰므로 `EvaluationResult` 스키마 확장으로 처리한다. 별도 노드를 만들면 Gemini 호출이 늘지만, 스키마만 넓히면 **호출 횟수는 그대로**고 출력 토큰만 증가한다
  - 검증: Calibration Set 17개로 **루브릭 도입 전후 정확도를 비교**한다. 목적이 사용자 납득만이 아니라 채점 편차 자체를 줄이는 것이므로, 수치로 확인할 수 있어야 한다
  - 주의: 모범답안은 `[Reference]` 범위 밖에서 생성되면 환각이 된다. Faithfulness를 0.4412에서 0.9708까지 올린 작업이 무의미해지지 않도록, 생성 범위를 제약하고 RAGAS로 재측정할 것
