...
- **Agent 분기가 단조로움**: `technical_score < 5` 하나로만 갈리고 점수가 높으면 아무 동작도 하지 않음. 또한 그래프에 사이클이 없어 LangGraph 채택 근거가 약함 (Phase 7, 멀티턴 루프로 대응 예정)
...
### Phase 7: Agent 다중 분기 (진행 예정)
현재 Agent는 `technical_score < 5` 하나로만 갈리고, 점수가 높으면 아무 동작도 하지 않아 분기가 단조롭다. 점수 구간별로 서로 다른 코칭을 제공하도록 확장한다.

- [ ] `decide_next_step()`을 3구간 분기로 확장
  - 0~3점: 기초 개념 설명 노드 (신규)
  - 4~6점: 기존 Learning Tip → Followup
  - 7~10점: 심화/압박 질문 노드 (신규)
- [ ] Docker 패키징 전에 완료해 배포 버전에 포함시킬 것

### Phase 8: 배포 및 인프라 (①~④, ⑥~⑦은 부트캠프 과제 / ⑤는 자체 추가)
