...
운영자가 직접 작성하는 고정 콘텐츠. 현재 **11개 문서**(jwt, fastapi, spring, postgresql, docker, http, oauth, caching, session_vs_token, async_sync, cors). 초기 계획은 20~30개였으나, 11개로 Retrieval 지표가 변별력을 갖는 것을 확인했고 (설계 이력 참고), 추가 확장은 다음 단계로 남겨둠 (Futeur Work). `scripts/load_kb.py`로 일회성 인덱싱.
...
| Embedding 비교 | 5개 질문 중 1개에서 차이 관찰 | `ko-sroberta-multitask` vs Gemini Embedding. KB 11개 기준 재실험에서, 헷갈리는 케이스(Access/Refresh Token 질문)에서만 Gemini Embedding이 더 안정적인 결과. 표본이 작아 일반화하기는 이름 |
...
- Retrieval 평가(Context Precision, Embedding 비교)의 표본이 5개 질문뿐이라, 결론을 일반화하기엔 이름. Retrieval 평가 전용 질문 세트 확장 필요
...
### 다음 단계 (진행 예정)
- [ ] Retrieval 평가 전용 질문 세트 확장 (현재 표본 5개 질문, 결론 확정에는 표본 부족)
