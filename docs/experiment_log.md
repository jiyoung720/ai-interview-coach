...
> 시간순 기록입니다. 항목마다 제목 아래 **한 줄 요약과 성격 태그**가 붙어 있어, 목차에서 눌러 한 줄만 읽고 계속 볼지 정할 수 있습니다.
>
> - **처음 보신다면**: 아래 [겪은 문제와 해결](#겪은-문제와-해결)에서 흥미로운 문제를 골라 그 기록으로 들어가세요
> - **핵심 결론만**: [README의 Key Findings](../README.md#key-findings)가 더 짧습니다
>
> 태그는 `트러블슈팅`(18) `측정`(5) `설계 결정`(3) `개선`(3) `관찰`(2) `검증`(2) `발견`(1) 입니다.
> 가설을 세우고 검증한 실험과, 하다가 문제를 발견해 파고든 기록이 섞여 있어 항목마다 구성이 조금씩 다릅니다.

## 목차

**Retrieval 품질** (16개) 검색이 의미로 작동하는가, 문서를 어떤 단위로 쪼갤 것인가
...
- [2026-08-06 - KB 확장이 임베딩 선택을 뒤집었다 (Phase 15)](#2026-08-06---kb-확장이-임베딩-선택을-뒤집었다-phase-15)
- [2026-08-06 (계속) - 근거가 없으면 채점하지 않는다 (Phase 17)](#2026-08-06-계속---근거가-없으면-채점하지-않는다-phase-17)
...
`트러블슈팅` 태그가 붙은 항목에서 실제로 부딪히고 해결한 것들을 모았다.

### 검색·평가 품질

| 문제 | 원인 | 해결 |
|---|---|---|
| Judge 채점 정확도가 52.9% | Judge가 질문 범위가 아니라 KB 문서 전체를 체크리스트처럼 사용 | 프롬프트에 "질문이 요구하지 않은 배경지식은 감점 사유가 아니다" 명시 → 58.8% ([기록](#2026-07-06---judge-calibration-자동화-및-completeness-프롬프트-개선)) |
| 프롬프트를 고쳐도 average 5건 전부 실패 | **같은 답변에 서로 다른 기대치**를 준 정답지 결함 | average를 "실제로 틀린 답변"으로 재작성 → 94.1% ([기록](#2026-07-07-계속---calibration-set-재설계-및-최종-검증)) |
| Faithfulness를 Chain A에 적용하려 함 | 면접 질문은 "주장"이 아니라 "질의"라 지표 전제와 안 맞음 | 대상을 Chain B로 재설정 ([기록](#2026-07-13-계속---ragas-faithfulness-적용)) |
| Top-1 정확도가 80%로 낮게 나옴 | MISS 4건 중 3건은 검색 실패가 아니라 **정답 라벨이 좁았던 것** | `expected_sources` 복수 라벨로 재설계 ([기록](#2026-07-14---retrieval-평가-전용-질문-세트-구축-및-실행)) |
| "트랜잭션 ACID"가 `oauth.md`를 1순위로 (3회 재현) | `postgresql.md`가 인덱스·트랜잭션 두 주제를 한 파일에 담음 | 두 파일로 분리 ([기록](#2026-07-15-계속---트랜잭션-acid-검색-오류-재현-확인)) |
| 문서 분리가 **역효과**를 냄 | 비교형 문서는 쪼개면 "비교"라는 문맥이 사라짐 | 비교 근거를 한 chunk에 유지 ([기록](#2026-07-15-계속---springmdsession_vs_tokenmd-분리-결과-성공과-실패가-갈림)) |
| 검색은 1위인데 Faithfulness 0.6667 | 문서에 그 근거가 아예 없었음 | 문서에 근거 보완 ([기록](#2026-07-18-계속---jwt-로그아웃-문서의-근거-범위-보완)) |
| 프롬프트를 네 번 고쳤는데 판정 불가 | 표본 17개의 **변동 폭이 개선 폭보다 큼** | 44개로 늘리고 3회 반복 → 변동 폭 5%p 확보 ([기록](#2026-07-31---채점-루브릭-도입과-측정-도구-재설계-phase-12)) |
| "멱등성"이 계속 엉뚱한 문서로 검색됨 | 문서가 아니라 **임베딩이 저빈도 전문용어를 모름** | Gemini Embedding으로 교체 (86.7% → 100%) ([기록](#2026-08-06---kb-확장이-임베딩-선택을-뒤집었다-phase-15)) |

### 빌드·배포

| 문제 | 원인 | 해결 |
|---|---|---|
| 도커 이미지가 10.9GB | GPU가 없는데 CUDA 스택 3.5GB가 설치됨 | 이미지에서만 CPU 전용 torch (10.9GB → 3.79GB) ([기록](#2026-07-21-계속---docker-패키징-이미지-크기-109gb에서-379gb로)) |
| CPU torch 지정이 무시됨 (재빌드해도 nvidia 잔존) | `tool.uv.sources`는 직접 의존성에만 적용. torch는 **전이 의존성** | torch를 직접 의존성으로 추가 + 인덱스 `explicit` 제한 ([기록](#2026-07-21-계속---docker-패키징-이미지-크기-109gb에서-379gb로)) |
| 빌드한 이미지가 EC2에서 실행되지 않음 | 맥은 arm64, EC2는 amd64 | `--platform linux/amd64` 지정, 이후 빌드를 CI로 이관 ([기록](#2026-07-21-계속---docker-패키징-이미지-크기-109gb에서-379gb로)) |
| dev 의존성이 배포 이미지에 포함될 뻔함 | `uv sync`가 기본으로 dev 그룹까지 설치 | `--no-dev` 추가 ([기록](#2026-07-21-계속---github-actions-ci-구성)) |
| EC2 빌드가 `no space left on device` | 이미지 2.7GB라 8GB면 충분하다 추정. **빌드에만 8.6GB 필요** | 20GB로 확장. 이미지 크기와 빌드 공간은 다른 값 ([기록](#2026-07-22---ec2-배포-메모리-측정으로-인스턴스-타입-결정-디스크-부족-대응)) |
| 배포마다 모델 449MB를 다시 받을 뻔함 | 레이어 순서상 코드 COPY 뒤에 있어 캐시가 무효화됨 | 모델 다운로드를 코드보다 앞으로 이동 (재빌드 1.6초) ([기록](#2026-07-22-계속---cd-구성-배포-자동화가-기존-설계를-되돌아보게-한-지점들)) |
| CD 첫 배포가 SSH timeout | 22번을 "내 IP만"으로 막았는데 **GitHub 러너는 다른 IP** | 22번 개방 (키 인증만 허용되므로 감수) ([기록](#2026-07-22-계속---cd-구성-배포-자동화가-기존-설계를-되돌아보게-한-지점들)) |
| CD가 갑자기 끊김 | EC2 재시작으로 퍼블릭 IP가 바뀜 | Secrets 갱신 (Elastic IP가 근본 해결) ([기록](#2026-07-26-계속---wireshark-http-통신-캡처-클라이언트-서버-분리)) |

### 프론트엔드

| 문제 | 원인 | 해결 |
|---|---|---|
| 화면에 `**` 같은 마크다운 서식이 노출 | `followup_node`만 structured output을 쓰지 않음 | 프롬프트에 "순수 텍스트로만 출력" 지시 ([기록](#2026-07-27-계속---프론트엔드-추가-phase-10)) |
| 고쳤는데 화면에 반영되지 않음 | **브라우저 캐시** (코드를 의심할 뻔했다) | 시크릿 창으로 캐시를 먼저 배제 ([기록](#2026-07-27-계속---프론트엔드-추가-phase-10)) |
| 라이트 모드인데 다크 색이 읽힘 | CSS `transition`이 진행 중인 **중간값**을 읽음 | `transition: none`을 임시 주입해 재측정 ([기록](#2026-07-28---프론트엔드-대화형-개편)) |
| 세션이 끝나도 입력창이 사라지지 않음 | `display:flex`가 `hidden`의 `display:none`을 이김 | `[hidden] { display: none !important }` ([기록](#2026-07-28---프론트엔드-대화형-개편)) |

### 환경·설정

| 문제 | 원인 | 해결 |
|---|---|---|
| RAGAS import 자체가 실패 | `ragas`가 아니라 **`langchain-community`가 최신**이었던 것 | 두 패키지를 함께 낮춤 ([기록](#2026-07-13-계속---ragas-faithfulness-적용)) |
| Gemini 임베딩 API 키 인식 실패 | `embeddings.py`가 `rag.config`를 import하지 않아 `.env` 미로딩 | `google_api_key`를 명시적으로 전달 ([기록](#2026-07-13-계속---embedding-비교-실험-ko-sroberta-multitask-vs-gemini-embedding)) |
| PDF에서 한글이 깨져 추출됨 | 우리 코드가 아니라 **`cupsfilter`가 만든 불량 PDF** | 정상 PDF로 다시 만들어 재검증 ([기록](#2026-07-27---pdf-업로드-지원-phase-9)) |
| checkpoint 직렬화 경고 | LangGraph 기본값이 "모든 타입 허용하되 경고" | 명시적 허용으로 변경 ([기록](#2026-07-27-계속---멀티턴-면접-루프-phase-11)) |

### 조용히 망가진 것들

에러도 안 나고 테스트도 통과하는데 실제로는 못 쓰는 상태였던 것들이다.

| 문제 | 원인 | 해결 |
|---|---|---|
| 엔드포인트 변경이 인프라 세 곳으로 번짐 | `GET /`를 데모 페이지로 쓰려는데 헬스체크가 점유 | `/health`로 분리 ([기록](#2026-07-27-계속---프론트엔드-추가-phase-10)) |
| 배포 검증이 헬스체크가 아니었음 | deploy job 갱신 누락. **세 곳인 줄 알았는데 네 곳이었다** | `/health`로 정정 ([기록](#2026-07-27-계속---멀티턴-면접-루프-phase-11)) |
| 새 Calibration Set에서 51.2% | `average` 라벨을 붙인 답변에 **틀린 내용이 없었음** | 케이스 재작성 → 90.7% ([기록](#2026-07-31---채점-루브릭-도입과-측정-도구-재설계-phase-12)) |
| 정답 키 오류를 RAGAS 수백 회 돌리고 발견 | `http.md` 분리 때 다른 문서로 잘못 매핑 | `key_evidence` 전수 검사 추가 ([기록](#2026-08-06-계속---근거가-없으면-채점하지-않는다-phase-17)) |
| `import`만 했는데 인덱스가 깨짐 | 스크립트가 **모듈 최상단에서 바로 실행**되는 구조 | `main` 가드 추가 ([기록](#2026-08-06-계속---근거가-없으면-채점하지-않는다-phase-17)) |
| EC2에서 모든 질문이 "근거 없음"으로 빠짐 | 인덱싱 대상과 앱의 검색 대상이 어긋남 | 같은 검색기를 따라가도록 수정 + 회귀 테스트 ([기록](#2026-08-06-계속---근거가-없으면-채점하지-않는다-phase-17)) |
| 2턴에 코칭이 두 개 표시됨 | State가 턴을 넘어 누적되는데 걸러내지 않음 | `next_action`으로 이번 턴 것만 선택 ([기록](#2026-08-06-계속---근거가-없으면-채점하지-않는다-phase-17)) |

---

## 2026-06-23 - Semantic Retrieval 검증

> `검증` 검색은 키워드가 아니라 의미로 작동했다. 다만 여러 주제가 섞인 긴 chunk가 부풀려진 유사도를 받는다는 것도 함께 드러났다.
...
> `발견` 검색이 정확해도 생성 모델이 근거 밖 내용을 덧붙였다. 검색 성공과 생성 충실도는 따로 재야 한다.
...
> `트러블슈팅` Judge가 질문 범위가 아니라 KB 문서 전체를 체크리스트처럼 쓰고 있었다. 프롬프트로 범위를 한정해 52.9% → 58.8%. 남은 실패의 원인은 정답지 결함이었다.
...
> `트러블슈팅` 정답지를 고치자 94.1%가 됐다. 남은 1건은 경계선 변동이라 판단하고, 범위를 넓혀 100%를 만드는 대신 거기서 멈췄다.
...
> `검증` 답변의 오류 강도와 정답 커버리지를 조절해 정확히 5점이 나오는 답변을 만들어, 분기 경계가 코드대로 동작함을 실행으로 확인했다.
...
> `설계 결정` 확장 방향을 기술적 가능성이 아니라 "왜 필요한지 설명할 수 있는가"로 골랐다. 병렬 대신 순차를 택한 것도 속도보다 일관성을 우선한 선택이다.
...
> `트러블슈팅` 지표를 Chain A에 적용하려던 것이 오류였다. 면접 질문은 "주장"이 아니라 "질의"라 Faithfulness의 대상이 아니다. Chain B로 재설정해 17개 평균 0.4412를 얻었다.
...
> `측정` 5개 질문 전부 1.0000이 나왔으나 KB가 2개 문서뿐이라 변별력이 없는 조건이었다. 만점을 성능이 아니라 "측정 조건 미충족"으로 판정했다.
...
> `측정` 두 임베딩이 완전히 같은 결과를 냈다. 성능이 같아서가 아니라 KB 규모가 차이를 드러낼 수 없는 조건이었기 때문이다. 결론을 유보했다.
...
> `측정` KB를 11개로 늘리자 처음으로 차이가 드러났다. Context Precision이 1.0000에서 0.8000으로 떨어진 것이 오히려 측정이 작동하기 시작했다는 신호였다.
...
> `트러블슈팅` 표본을 5개에서 20개로 늘리자 Top-1이 80%로 떨어졌다. 그런데 MISS 4건 중 3건은 검색 실패가 아니라 정답 라벨을 하나로 좁게 붙인 탓이었다.
...
> `트러블슈팅` 3회 모두 재현됐다. 원인은 postgresql.md가 인덱스와 트랜잭션 두 주제를 한 파일에 담고 있었던 것. "파일당 주제 하나"로는 부족하고 "chunk당 주제 하나"가 필요했다.
...
> `개선` 문서를 둘로 나누자 ACID 질문이 정상화됐다. 한 번에 하나만 바꾸는 실험으로 다른 문항에 영향이 없음도 함께 확인했다.
...
> `개선` 정답이 하나가 아닌 질문에 복수 라벨을 허용해 Top-1 100%가 됐다. 다만 이 100%를 "검색이 완벽하다"로 읽지 않았고, source는 맞는데 근거 점수가 낮은 새 문제를 발견했다.
...
> `트러블슈팅` 검색은 맞았는데 근거 점수가 낮았던 원인은 chunking이었다. 제목만 남은 chunk와 근거가 흩어진 chunk가 문제였다.
...
> `트러블슈팅` 같은 처방이 문서에 따라 반대 결과를 냈다. 독립 개념 나열 문서는 쪼갤수록 좋아졌고, 비교형 문서는 쪼개자 "비교"라는 문맥이 사라져 나빠졌다.
...
> `개선` 비교 근거를 한 chunk에 묶자 회복됐다. 분리 단위의 기준이 파일 크기가 아니라 "질문 하나에 답할 근거가 흩어지지 않는 단위"임을 확인했다.
...
> `트러블슈팅` 검색은 1위인데 Faithfulness가 0.6667이었다. 문서에 그 근거가 아예 없었기 때문이다. 검색 순위와 근거 충분성은 다른 문제다.
...
> `측정` 5문항에서 우세했던 Gemini가 20문항에서 뒤집혔다. 작은 표본의 결론을 일반화하면 안 된다는 것이 실증됐다. 스스로 "이 결과도 최종이 아니다"라는 유보를 남겼다.
...
> `설계 결정` 분기를 늘린 게 아니라 "구간마다 필요한 코칭이 다르다"를 기준으로 셋으로 나눴다. 빈 경로가 사라졌고, 분기 로직을 LLM 없이 전수 검증했다.
...
> `트러블슈팅` 이미지가 10.9GB였다. GPU가 없는데 CUDA 스택 3.5GB가 들어 있었다. CPU 전용 torch로 3.79GB까지 줄였는데, 첫 시도는 에러 없이 실패해 크기로만 알아챌 수 있었다.
...
| `torch` | 914MB | 필요 |
| 기타(pyarrow, scipy, transformers 등) | 약 1.2GB | 필요 |
...
> `트러블슈팅` CI에 API 키가 없다는 제약이 오히려 "키 없이 검증 가능한 계층"을 분리하게 했다. dev 의존성이 배포 이미지에 들어갈 뻔한 것도 함께 잡았다.
...
> `트러블슈팅` 인스턴스 타입을 측정으로 정했다. 빌드는 디스크 부족으로 실패했는데, 이미지 2.7GB와 빌드에 필요한 8.6GB는 전혀 다른 값이었다.
...
> `트러블슈팅` 자동화가 기존 설계의 전제를 깼다. "SSH는 사람만 쓴다"는 가정이 무너졌고, 레이어 순서 때문에 배포마다 모델을 다시 받을 뻔했다.
...
> `측정` 병목은 서버도 네트워크도 아닌 LLM 대기였다. 사양을 올려도 빨라지지 않는다는 뜻이다. 순차 설계의 대가도 약 4-6초로 수치화됐다.
...
> `관찰` 동시 요청 5개가 거의 동시에 끝났다. I/O 바운드는 CPU가 놀면서 여러 대기를 떠안을 수 있기 때문이다. 직접 쓴 KB 문서의 내용을 그 KB를 서빙하는 서버에서 확인한 셈이다.
...
> `관찰` 요청 하나의 생애를 13패킷으로 봤다. 요청·응답 JSON이 평문으로 그대로 읽히는 것도 직접 확인해, HTTPS 도입의 근거가 됐다.
...
> `트러블슈팅` 한글이 깨져 나왔는데 원인이 우리 코드가 아니라 테스트용 PDF 자체였다. 입력을 의심하기 전에 코드부터 뒤졌다면 한참 헤맸을 것이다.
...
> `트러블슈팅` 화면에 붙이자 백엔드에서 안 보이던 문제가 드러났다. structured output을 안 쓰는 노드에서만 서식이 샜고, 엔드포인트 한 줄 변경이 인프라 세 곳으로 번졌다.
...
> `설계 결정` "그럴 거면 LCEL로도 되지 않나"에 답한 편이다. 사이클과 중단·재개는 LCEL로 만들 수 없다. judge 호출 횟수를 세어 사이클을 증명했다.
...
> `트러블슈팅` 검증 도구가 만든 착시를 두 번 겪었다. transition 중간값을 버그로 오해했고, display:flex가 hidden을 무력화하는 회귀도 만들었다.
...
> `트러블슈팅` 프롬프트를 네 번 고쳤지만 무엇도 판정하지 못했다. 측정 도구의 해상도가 변화보다 거칠었기 때문이다. 같은 함정(정답지 결함)에 두 번째로 빠졌다.
...
## 2026-08-06 - KB 확장이 임베딩 선택을 뒤집었다 (Phase 15)

> `트러블슈팅` 문서를 고쳐도 "멱등성" 검색이 안 됐다. 원인은 임베딩이 그 단어를 모른다는 것이었다. 문서로는 풀 수 없는 종류의 문제를 처음 만났다.

### 배경
KB를 18개에서 29개 문서로 늘린 뒤(DB 심화 4개, 웹/네트워크 3개, `http.md` 분리 5개), 검색 품질을 재측정하자 Top-1 정확도가 떨어졌다. 특히 "멱등성이란 무엇인가요?" 질문이 `async_sync.md`로 새는 문제가 있었다. `http_idempotency.md` 전용 문서를 만들고 제목에 동의어("같은 요청을 여러 번 보내도 결과가 같은 성질")까지 넣었는데도 **거리가 0.6969에서 전혀 움직이지 않았다.**

### 원인 규명 - 문서가 아니라 임베딩의 어휘 문제였다
단어 쌍의 코사인 거리를 직접 재봤다.

| 단어 A | 단어 B | ko-sroberta | Gemini |
|---|---|---|---|
| 멱등성 | 같은 요청을 여러 번 보내도 결과가 같은 성질 | 0.1682 | 0.7942 |
| 멱등성 | 비동기 처리 (무관) | 0.4091 | 0.6348 |
| 트랜잭션 | 여러 작업을 하나로 묶어... | 0.3817 | 0.7501 |

ko-sroberta는 **정의보다 무관한 단어를 더 가깝게 보는 역전**이 일어난다. "트랜잭션"은 정상이므로 모델 전반의 문제가 아니라 저빈도 전문용어에 국한된 한계다.

지금까지 이 프로젝트가 검색 품질을 다룬 방식은 전부 KB 쪽이었다(chunk 분리, retrieval unit 재구성, 혼합 주제 분리). **문서를 어떻게 써도 해결되지 않는 종류의 문제를 처음 만났다.**

### 결과 - 30문항 전체 재측정

| | 7월 (KB 18개, 20문항) | 지금 (KB 29개, 30문항) |
|---|---|---|
| ko-sroberta | **100%** / F 0.9708 | 86.7% / F 0.8931 |
| Gemini | 95% / F 0.9500 | **100%** / F 0.9702 |

ko-sroberta가 틀린 4문항은 전부 이번에 추가한 DB 심화 주제였다.

| 질문 | ko-sroberta | Gemini |
|---|---|---|
| 반정규화는 언제 적용? | `caching.md` | `db_normalization.md` |
| N+1 문제가 무엇? | `http_status_code.md` | `db_n_plus_one.md` |
| 지연 로딩을 즉시 로딩으로? | `async_sync.md` | `db_n_plus_one.md` |
| 동시에 같은 데이터 수정? | `db_normalization.md` | `db_lock.md` |

"N+1 문제"를 `http_status_code.md`로 보낸 것은 오차가 아니다. 숫자 `N`과 `+`라는 **표면 형태에 반응한 것**이지 의미를 이해한 결과가 아니다.

**KB 18개일 때는 주제가 서로 멀어 어휘를 몰라도 갈렸다. 29개가 되자 `db_normalization`/`db_n_plus_one`/`db_lock`처럼 가까운 주제가 생겼고, 거기서 용어를 진짜 아는지가 드러났다.** 7월 결론이 틀렸던 게 아니라 그때 조건에서만 맞았다.

### 채점에는 영향이 없었다
임베딩 교체는 채점자가 보는 근거 문서를 바꾸므로 채점 결과도 재측정해야 한다. 다만 44케이스를 전부 돌리기 전에, 검색만 비교해 영향 범위를 먼저 좁혔다(LLM 호출 없이 임베딩만). 15개 고유 질문 중 3개에서 1순위가 갈렸고 해당 케이스는 7개, 최대 변동 폭 15.9%p로 노이즈(5%p)보다 커서 재측정할 값어치가 있었다.

| | 경로 정확도 |
|---|---|
| Phase 12 (ko-sroberta) | 93.2%, 88.6%, 93.2% |
| 지금 (Gemini) | 90.9% |

**노이즈 범위 안이다.** 검색이 13.3%p 좋아졌는데 채점 정확도는 안 움직였다. 원인은 Judge가 답변 자체를 보고 채점하기 때문이다. Access/Refresh Token 질문이 `spring_layered_architecture.md`를 근거로 채점되던 상황에서도 3개 케이스 모두 정확했다. Phase 12의 "루브릭을 빼도 점수가 같다"와 같은 결의 발견이다.

### 대가
| | ko-sroberta | Gemini |
|---|---|---|
| 검색 지연 (중앙값) | 9.0 ms | 524.4 ms |
| 검색당 비용 | 0 | 과금 |
| 오프라인 | 동작 | 불가 |
| 벡터 차원 | 768 | 3072 |

58배 느리지만 한 턴 전체가 LLM 생성으로 수 초 걸리므로 체감 차이는 작다. 실제 대가는 **의존성**이다. 이날 크레딧이 소진되자 검색부터 죽었다. 이전에는 채점만 실패하고 검색은 살아 있었다.

### 결론
1. **Gemini Embedding 채택.** 근거는 정확도 13.3%p가 아니라, 사용자에게 보이는 `retrieved_sources`가 엉뚱하게 뜨는 결함이 사라진다는 점이다. 점수가 맞아도 "N+1 질문에 HTTP 상태 코드 문서 참고"라고 뜨면 신뢰가 무너진다
2. **표본/조건을 바꾸면 결론이 뒤집힌 세 번째 사례다.** 5→20문항(임베딩), 17→44케이스(루브릭), 그리고 이번(KB 18→29개). 7월 기록에 "이번 결과 역시 최종 진리로 단정하지 않는다"고 남겼던 유보가 실제로 맞았다
3. **검색 개선이 최종 지표로 이어지지 않을 수 있다.** 중간 지표(Top-1)와 최종 지표(채점 정확도)를 구분해서 말해야 한다

### 이날 저지른 실수
- **RAGAS 비용을 고려하지 않고 백그라운드로 세 번 연속 실행**해 크레딧을 소진시켰다. RAGAS는 지표 하나에 LLM을 여러 번 부르므로 문항당 약 6배로 불어난다. 이후 평가 스크립트에 `--limit`과 예상 호출량 출력을 추가했다
- **"임포트만 하는 검사"가 무료가 아니었다.** `scripts/` 하위 스크립트 상당수가 모듈 최상단에서 바로 실행되는 구조라 `import scripts.load_kb_gemini` 한 줄이 KB 재인덱싱을 시작했고, 중간에 한도에 걸려 `async_sync.md`가 인덱스에서 통째로 빠졌다. 8개 스크립트에 `if __name__ == "__main__"` 가드를 넣고, `--only-missing`으로 빠진 문서만 채우는 경로를 만들었다
  - 부수 효과로 손상 상태에서 "비동기 처리" 질문이 `fastapi.md`로 새는 것을 관찰했고, 복구 후 `async_sync.md`(거리 0.2210)로 돌아오는 것까지 확인했다. Faithfulness도 0.9591에서 0.9702로 회복됐다
- **평가셋 정답 키 오류를 발견했다.** "JWT를 HTTP 요청에 실어 보낼 때 어떤 헤더를 사용하나요?"의 정답이 `rest_api_design.md`로 돼 있었으나 그 문서에는 `Authorization`도 `Bearer`도 없다. `http.md`를 분리할 때 잘못 매핑한 것으로, 두 임베딩 모두 실제로는 `authorization_header.md`를 찾아오고 있었다

### Action Item
- `senior` 레벨이 7건 나왔다(전부 tech=10 comp=10). Phase 13에서 "senior가 안 나온다"고 남긴 과제와 상반되나, 임베딩 교체 덕인지 Phase 12에서 캘리브레이션 세트를 손본 뒤 이미 그랬는지는 이 측정만으로 알 수 없다. 원인은 단정하지 않는다
- **평가셋이 천장에 닿았다.** Top-1 100%, Context Precision 1.0000이라 다음 개선(KB 문서 보강)의 효과를 측정할 수 없다. KB를 보강하기 전에 평가셋부터 어렵게 만들어야 한다(저빈도 전문용어, 인접 주제 구분, 정의형/비교형/적용형)
- 코칭 콘텐츠(학습 팁·개념 설명)의 품질은 검색된 문서를 재료로 하는데 아직 따로 측정하지 않았다

---

## 2026-08-06 (계속) - 근거가 없으면 채점하지 않는다 (Phase 17)

> `트러블슈팅` 근거가 없어도 항상 3개를 돌려주던 검색을 거리로 걸러냈다. 이날 잡은 버그 셋은 전부 200으로 응답하고 테스트도 통과하는 종류였다.

### 배경
Phase 15에서 임베딩을 교체한 뒤 평가셋이 천장에 닿았다(Top-1 100%, Context Precision 1.0000). 개선을 재려면 평가셋부터 어렵게 만들어야 했는데, 문항을 살펴보다 더 큰 구멍을 발견했다. **30문항 전부가 KB에 답이 있는 질문이었다.**

검색은 `k=3`으로 항상 3개를 돌려준다. KB에 근거가 없어도 무언가는 나오고, 그것으로 채점한다.

| 질문 | 1순위 거리 | 1순위 문서 |
|---|---|---|
| 멱등성이란 무엇인가요? | 0.231 | `http_idempotency.md` |
| Kubernetes Pod 오토스케일링은? | 0.402 | `docker.md` |
| 좋아하는 음식이 무엇인가요? | 0.424 | `async_sync.md` |

시연에서 생성된 면접 질문에도 "Redis TTL 전략", "Session Clustering"처럼 KB가 다루지 않는 주제가 실제로 있었다.

### 정책을 먼저 정했다
이것을 검색 실패로 보고 임계값부터 정하려 했으나, **제품 정책이 먼저**라는 지적을 받고 순서를 바꿨다. 기대 동작을 정하지 않은 채로 측정하면 무엇을 실패라 부를지부터 없다.

- 근거가 부족하면 `out_of_scope`로 분기해 **점수·level·학습 팁·꼬리질문을 만들지 않는다**
- 범위 밖 턴은 `MAX_TURNS`에 세지 않는다. 채점받지 못한 턴을 면접 턴으로 볼 이유가 없다
- 이어갈지는 사용자가 정한다. 질문 하나가 KB 밖이었을 뿐 면접이 끝난 것은 아니다

### 평가셋을 두 번 만들었다
범위 밖 19문항(Kafka, Kubernetes, Elasticsearch 등)을 넣고 재보니 **완벽하게 갈렸다.**

```
범위 안 0.166~0.271  |  범위 밖 0.324~0.412     겹침 없음
```

좋은 신호로 보였지만 아니었다. **KB에서 뻔히 먼 주제만 골랐기 때문**이다. 어떤 임계값을 넣어도 100%가 나오면 임계값이 검증된 것이 아니라 시험이 쉬운 것이다.

그래서 KB 가장자리에 걸치는 8문항을 추가했다(JPA 영속성 컨텍스트, SQL 인젝션, HTTP 캐시 헤더, Spring AOP 등). 그제야 겹침이 생겼다.

```
[안] 0.373  재고가 실제보다 많이 남아 있는 것으로 집계됩니다  -> db_lock.md
[밖] 0.321  프로세스와 스레드의 차이는 무엇인가요?          -> async_sync.md
```

두 값이 역전돼 완벽한 임계값이 사라졌고, 그때부터 임계값 선택이 트레이드오프가 됐다.

### 임계값 산출 - group 5-fold
같은 주제를 표현만 바꾼 문항이 train/test에 걸쳐 들어가면 임계값이 실제보다 좋아 보이므로, 무작위가 아니라 `topic` 단위로 묶어 나눴다.

| fold | 임계값 | 재현율 | 정밀도 | 범위 안 통과 |
|---|---|---|---|---|
| 1 | 0.309 | 100% | 83.3% | 91.7% |
| 2-3, 5 | 0.311 | 100% | 100% | 100% |
| 4 | 0.309 | 100% | 85.7% | **75.0%** |
| 평균 | | 100% | 93.8% | 93.3% |

전체 데이터로는 오탐 1건뿐이지만 **fold 4에서 75%로 떨어진다.** 전체만 봤으면 넘어갔을 지점을 교차 검증이 드러냈다.

선택 기준은 오탐 억제다. 범위 안을 잘못 막으면 지금보다 나빠지지만, 범위 밖을 놓치는 것은 현재 동작과 같기 때문이다.

**불확실성의 종류가 다르다는 점도 정리했다.** Phase 12에서 잰 변동 폭 5%p는 같은 입력에 LLM이 다르게 답해 생긴 값이다. 거리 판정은 결정적이라 재실행 변동이 0이고 남는 것은 표본오차뿐이다(정확도 0.9 근처에서 n=30이면 약 5.5%p). 둘을 섞어 판단하면 안 된다.

### 결과
평가셋 30 → 65문항, 5유형(in_scope 27 / out_of_scope 27 / paraphrase 8 / multi_source 3).

`key_evidence`(정답 문서 안에 실제로 있는 문구)를 전 문항에 넣고 전수 검사를 붙였다. Phase 15에서 정답 키가 `rest_api_design.md`로 잘못 매핑된 것을 RAGAS 수백 회 돌리고 나서야 발견했는데, 이제 0.01초에 걸린다. `고립성`을 `격리성`으로 바꾸는 정도의 오타도 잡힌다.

### 이날 잡은 버그 세 개 - 전부 조용히 망가지는 종류였다

**1. 이전 턴의 코칭이 이번 턴 결과로 샜다**

브라우저로 3턴 전체를 돌려보다 발견했다. 2턴이 advanced 경로인데 1턴의 학습 팁이 같이 렌더됐다.

```
턴 1  학습 팁
턴 2  학습 팁 + 심화 질문 출제    <- 1턴 것이 남아 있음
```

LangGraph State는 턴을 넘어 누적되는데 코칭 키의 존재만 보고 응답에 담고 있었다. `next_action`으로 이번 턴에 실행된 경로의 키만 고르도록 고쳤다. **API 응답은 200이고 스키마도 유효해서, 화면을 직접 보지 않았으면 못 잡았다.**

**2. 인덱싱 대상이 앱의 검색 대상과 어긋났다**

배포 직전에 발견했다. 앱은 Gemini 컬렉션을 검색하는데 `docker-entrypoint.sh`는 ko-sroberta 컬렉션만 확인했다. EC2 볼륨에는 옛 ko-sroberta 인덱스가 이미 차 있으므로 **"인덱싱 완료"로 판단하고 건너뛴 뒤, 앱은 빈 컬렉션을 검색**하게 된다.

```
검색 0건 -> top_distance = 1.0 -> 임계값 초과 -> 모든 질문이 "평가 보류"
```

크래시가 아니라 200으로 잘 응답한다. 헬스체크도 통과한다. 화면에는 "검증 가능한 근거가 없습니다"만 계속 뜬다. 컬렉션을 지목하는 대신 `get_interview_kb_retriever()`를 따라가도록 고쳐, 임베딩을 또 바꿔도 어긋나지 않게 했다.

**3. CI 스모크 테스트가 깨졌다 (2번을 고치며 만든 회귀)**

CI는 더미 키로 컨테이너를 띄워 기동을 확인한다. 인덱싱이 Gemini를 호출하게 되면서 400이 나 컨테이너가 죽었다. 워크플로 주석에 전제가 적혀 있었는데(`KB 인덱싱은 Gemini를 호출하지 않으므로 API 키 없이 가능하다`) 그 전제를 깬 것이다. `SKIP_KB_LOAD=1`로 분리했다. 여기서 확인할 것은 이미지가 떠서 헬스체크에 응답하는가이지 인덱싱이 아니다.

### 배포 검증
성공 로그만으로는 2번 같은 고장을 알 수 없어, EC2에서 직접 확인했다.

| 확인 | 결과 |
|---|---|
| 멱등성 질문 (범위 안) | 채점됨, `http_idempotency.md` 참조 |
| N+1 질문 (KB 확장분) | 채점됨, `db_n_plus_one.md` 참조 |
| Kafka 질문 (범위 밖) | 보류, 점수 없음 |
| 멀티턴 1→2턴 | 코칭이 턴마다 하나씩 |

### 배운 점
1. **기대 동작을 정하기 전에는 측정할 수 없다.** 처음에는 negative 문항을 넣고 실패율을 보려 했으나, "범위 밖일 때 앱이 뭘 해야 하는가"를 정하지 않으면 무엇을 실패라 부를지부터 정해지지 않는다
2. **완벽한 분리는 좋은 신호가 아니다.** 겹침이 없다는 것은 임계값이 검증됐다는 뜻이 아니라 경계 사례가 없다는 뜻이었다. 어려운 문항을 넣고 나서야 판단할 거리가 생겼다
3. **평가셋을 설계한 사람의 감각도 검증 대상이다.** Phase 12에서 라벨로 겪은 것과 같은 함정을 문항 난이도에서 또 만났다
4. **조용히 망가지는 고장은 자동 검사로 안 걸린다.** 세 버그 모두 200으로 응답하고 테스트도 통과했다. 브라우저로 직접 보고, 배포 후 실제로 질문을 던져보고 나서야 드러났다
5. **불확실성의 종류를 구분해야 한다.** LLM 채점의 재실행 변동과 결정적 판정의 표본오차는 계산 방식이 다르다

### Action Item
- 범위 밖 판정을 LLM 관련성 판단으로 바꾸는 방안은 보류했다. 임베딩을 바꿀 때마다 임계값을 다시 잡아야 하는 것이 거리 방식의 단점이지만, 그 재보정이 버전 관리 가능한 명시적 절차라는 장점도 있다. 평가셋을 고정해뒀으므로 이후 두 방식을 같은 기준으로 비교할 수 있다
- 65문항에 주제가 47개라 그룹당 1.4문항이다. 같은 주제 문항을 더 넣어야 group 분할의 효과가 제대로 나온다
- 범위 밖 재현율 100%는 아직 의심스럽다. 진짜 어려운 범위 밖 문항이 `프로세스와 스레드`(0.321) 하나뿐이었다
