...
원인은 파악됐다. **만점에서 깎게 하니 LLM이 깎을 이유를 적극적으로 찾았다.** good 케이스가 전부 10점에서 8점으로 내려앉았다. 반대로 "억지로 깎지 마라"를 넣자 이번엔 bad 케이스가 위로 샜다. 한쪽을 누르면 반대쪽이 올라오는 상태가 반복됐다.

### 진짜 문제 - 측정 도구의 해상도가 부족했다
...
1. **기대 범위를 분기 경계에 정렬**: 기존 세트는 `average`의 기대치가 `[1,4]`, `[2,5]`, `[3,6]`으로 제각각이었다. `[1,4]`면 1점도 통과인데 1점은 실제로 fundamentals 경로로 간다. 점수는 맞았다고 나오지만 엉뚱한 코칭이 나가는 상태다. 그래서 bad `[0,3]`, average `[4,6]`, good `[7,10]`으로 통일해 **분기 경계(4점, 7점)와 맞췄다.**
2. **주제 확대**: 5개 → 13개. KB 18개 문서 중 5개 주제만 검증하고 있었다.

이렇게 하니 **"점수가 맞았는가"가 아니라 "올바른 코칭이 나갔는가"를 잴 수 있게 됐다.** 사용자에게 실제로 영향을 주는 것은 몇 점인지가 아니라 어떤 코칭을 받느냐다.
...
### 감점 합계 검증
프롬프트에서 `deductions의 points 합 = 10 - technical_score`를 요구했지만 LLM이 산술을 틀릴 수 있어 따로 측정했다. **17개 케이스 전부 일치(100%)했다.**

감점 항목을 `technical_score`보다 앞에 배치한 스키마 설계가 작용한 것으로 보인다. 깎을 것을 먼저 나열하고 그 합을 10에서 빼는 순서라, 점수를 먼저 낸 뒤 감점을 사후에 맞추는 것보다 어긋날 여지가 적다.

다만 레벨 분포가 `junior` 13건, `middle` 4건으로 `senior`가 한 건도 없었다. v1 세트에 좋은 답변이 적은 탓일 수도 있고 판정 기준이 빡빡한 탓일 수도 있어, 판단은 보류한다.
